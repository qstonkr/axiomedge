...
class TestGraphFunctions:
    def test_get_graph_integrity(self):
        with patch("services.api.admin._get", return_value={"ok": True}) as m:
            result = admin.get_graph_integrity()
            m.assert_called_once_with("/api/v1/admin/graph/integrity")
            assert result == {"ok": True}

    def test_run_graph_integrity_check(self):
        with patch("services.api.admin._post", return_value={"ok": True}) as m:
            result = admin.run_graph_integrity_check()
            m.assert_called_once_with("/api/v1/admin/graph/integrity/run")

    def test_get_graph_stats(self):
        with patch("services.api.admin._get", return_value={"nodes": 10}) as m:
            admin.get_graph_stats()
            m.assert_called_once_with("/api/v1/admin/graph/stats")

    def test_get_graph_communities(self):
        with patch("services.api.admin._get", return_value={}) as m:
            admin.get_graph_communities()
            m.assert_called_once_with("/api/v1/admin/graph/communities")

    def test_graph_search_basic(self):
        with patch("services.api.admin._post", return_value={"nodes": []}) as m:
    ...
    def test_graph_search_with_node_types(self):
        with patch("services.api.admin._post", return_value={}) as m:
            admin.graph_search("q", node_types=["Person"])
            body = m.call_args[0][1]
            assert body["node_types"] == ["Person"]

    def test_graph_search_clamps_max_nodes(self):
        with patch("services.api.admin._post", return_value={}) as m:
            admin.graph_search("q", max_nodes=999)
            body = m.call_args[0][1]
            assert body["max_nodes"] == 200  # clamped

    def test_graph_search_clamps_max_hops(self):
        with patch("services.api.admin._post", return_value={}) as m:
            admin.graph_search("q", max_hops=10)
            body = m.call_args[0][1]
            assert body["max_hops"] == 5  # clamped

    def test_graph_search_clamps_low(self):
        with patch("services.api.admin._post", return_value={}) as m:
    ...
    def test_graph_expand_basic(self):
        with patch("services.api.admin._post", return_value={}) as m:
    ...
    def test_graph_health(self):
        with patch("services.api.admin._get", return_value={"status": "ok"}) as m:
            admin.graph_health()
    ...
    def test_graph_ai_classify_defaults(self):
        with patch("services.api.admin._post", return_value={}) as m:
    ...
    def test_graph_ai_classify_with_params(self):
        with patch("services.api.admin._post", return_value={}) as m:
...
class TestPipelineFunctions:
    ...
    def test_trigger_kb_sync_full(self):
        with patch("services.api.admin._post", return_value={}) as m:
    ...
    def test_publish_experiment_execute(self):
        with patch("services.api.admin._post", return_value={}) as m:
