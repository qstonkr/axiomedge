...
- **처리방침 고지**: 첫 로그인 시 `PrivacyConsent` 모달로 안내. accept 시 서버에 `chat_privacy_consents` row 기록 (legal trail) + `localStorage`(axe-privacy-consent-v1) 캐시. 클라이언트는 mount 시 `GET /api/v1/users/me/consent` 로 서버 상태 확인 후 modal 표시 여부 결정.
- **동의 철회 (PIPA §37)**: ProfileDropdown 의 "🚫 처리방침 동의 철회" 항목 또는 `DELETE /api/v1/users/me/consent` 로 철회. row 의 `withdrawn_at` 컬럼이 set 되고, 다음 chat 진입 시 PrivacyConsent modal 이 다시 떠 재동의를 요구. 기존 대화는 자동 삭제되지 않음 (사용자가 좌측 sidebar 에서 직접 삭제). 재동의 시 같은 row 의 `withdrawn_at` 이 null 로 clear 되며 단일 legal-trail row 가 유지됨.
