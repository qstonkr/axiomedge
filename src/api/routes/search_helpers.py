...
# Cache key 에 organization_id 를 포함시켜 cross-tenant leak 방지.
_KB_REGISTRY_CACHE_TTL_S = 60.0
_kb_registry_cache: dict[tuple[str, str | None], tuple[float, set[str]]] = {}


async def get_active_kb_ids(
    kb_registry, organization_id: str | None = None,
) -> set[str]:
    """Return currently-active KB id set, scoped to one organization if given.

    Cache key 는 (registry instance id, organization_id) — 동일 registry
    인스턴스라도 다른 org 요청은 별도 캐시 슬롯을 사용해 KB 가 다른 tenant
    로 새지 않게 보장.
    """
    cache_key = (str(id(kb_registry)), organization_id)
    now = time.monotonic()
    cached = _kb_registry_cache.get(cache_key)
    if cached and now - cached[0] < _KB_REGISTRY_CACHE_TTL_S:
        return cached[1]

    accessible_kbs = await kb_registry.list_all(organization_id=organization_id)
