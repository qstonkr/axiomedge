...
export const CONNECTOR_CATALOG: readonly ConnectorEntry[] = [
  ...
  },
  {
    ...
    configSchema: JSON.stringify(
      {
        folder_id: "0Adb_xxx_xxx (또는 'root')",
        recursive: true,
        max_files: 500,
      },
      null,
  ...
  },
  {
    ...
    category: "office",
    icon: "📊",
    description: "Google Sheets — 시트별 grid → markdown table (행/열 의미 보존)",
    status: "available",
    scope: "both",
    userTokenMode: "shared",
    configSchema: JSON.stringify(
      {
        spreadsheet_ids: ["1AbCdEf...", "1GhIjKl..."],
  ...
  },
  {
    ...
    category: "office",
    icon: "✉️",
    description: "Gmail 메시지 (검색 query 기반, domain-wide delegation 권장)",
    status: "available",
    scope: "both",
    userTokenMode: "shared",
    configSchema: JSON.stringify(
      {
        ...
        query: "from:boss after:2026-01-01",
        max_messages: 200,
        include_body: true,
      },
      null,
