    ...
    with st.form("create_group_form"):
        ...
        if submitted:
            if not new_name:
                ...
                result = api_client._request("POST", "/api/v1/search-groups", json_body={
                    "name": new_name,
        ...
        with st.container(border=True):
            ...
            with action_col:
                if st.button("🗑️ 삭제", key=f"del_{group_id}", use_container_width=True):
                    result = api_client._request("DELETE", f"/api/v1/search-groups/{group_id}")
                    if not api_failed(result):
            ...
            with st.expander(f"✏️ '{group_name}' 수정"):
                with st.form(f"edit_{group_id}"):
                    ...
                    if st.form_submit_button("저장"):
                        result = api_client._request("PUT", f"/api/v1/search-groups/{group_id}", json_body={
                            "name": edit_name,
