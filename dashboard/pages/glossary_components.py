        ...
        for m in matches:
            ...
            if morph_matches and len(morph_matches) > 1:
                ...
                for mm in morph_matches:
                    ...
                    parts.append(
                        f"{mm['standard_term']}({ko})" if ko else mm["standard_term"]
                    )
            ...
            if cs:
                ...
                row["S_sparse"] = (
                    f"{cs.get('sparse', 0):.3f}" if cs.get("sparse") else "-"
                )
                row["S_dense"] = (
                    f"{cs.get('dense', 0):.3f}" if cs.get("dense") else "-"
                )
        ...
        if cur_page > 1:
            if st.button("⬅️ 이전 페이지", key="sim_prev_page"):
                ...
                with st.spinner(f"페이지 {new_page} 분석 중..."):
                    result = api_client.check_pending_similarity(
                        threshold, page=new_page, page_size=50
                    )
                    if not api_failed(result):
                        st.session_state["sim_check_result"] = result
                        st.rerun()
            ...
            if st.button(
                ...
                for m in synonym_candidates:
                    ...
                    if not api_failed(res):
                        success_count += 1
                st.success(
                    f"{success_count}/{len(synonym_candidates)}건 동의어 등록 완료"
                )
                if "sim_check_result" in st.session_state:
                    del st.session_state["sim_check_result"]
                st.cache_data.clear()
    ...
    if review_matches:
        with st.expander(
            ...
            for idx, m in enumerate(review_matches):
                ...
                std_display = (
                    f"{matched_std} ({matched_std_ko})" if matched_std_ko else matched_std
                )
                ...
                with rc3:
                    if st.button("동의어 등록", key=f"syn_review_{idx}"):
                        ...
                        if not api_failed(res):
                            st.success(
                                f"'{pending_term}' → '{matched_std}' 동의어 등록 완료"
                            )
                            st.cache_data.clear()
                            st.rerun()
                        else:
        ...
        if not api_failed(disc_result):
            ...
            if disc_synonyms:
                ...
                for d_idx, disc in enumerate(disc_synonyms):
                    ...
                    base_terms = (
                        ", ".join(disc_synonyms_list) if disc_synonyms_list else "-"
                    )
                    ...
                    with st.container(border=True):
                        ...
                        with dc3:
                            ...
                            with dac1:
                                if st.button(
                                    "승인", key=f"disc_approve_{d_idx}", type="primary"
                                ):
                                    res = api_client.approve_discovered_synonyms(
                                        [disc_id]
                                    )
                                    if not api_failed(res):
                                        st.success("승인 완료")
                            ...
                            with dac2:
                                if st.button("거부", key=f"disc_reject_{d_idx}"):
                                    res = api_client.reject_discovered_synonyms(
                                        [disc_id]
                                    )
                                    if not api_failed(res):
                                        st.success("거부 완료")
...
def render_pending_list():
    ...
    if api_failed(pending_result):
        ...
        pending_total_pages = max(
            1, (pending_total + PENDING_PAGE_SIZE - 1) // PENDING_PAGE_SIZE
        )
        ...
        if pending_terms:
            ...
            for term in pending_terms:
                ...
                source_icon = {"AUTO": "🤖", "MANUAL": "✋", "LLM": "🧠"}.get(
                    source, ""
                )
                ...
                with st.container(border=True):
                    ...
                    st.write(definition)
                    st.caption(
                        f"소스: {source} | 카테고리: {category} | ID: {term_id}"
                    )
                    ...
                    with btn_col1:
                        if st.button(
                            "✅ 승인", key=f"approve_{term_id}", type="primary"
                        ):
                            result = api_client.approve_glossary_term(
                                term_id, approved_by="admin"
                            )
                            if api_failed(result):
                                st.error("승인 실패")
                            else:
...
def render_expansion_tab():
    ...
    approved_terms = expansion_result.get(
        "items", expansion_result.get("terms", [])
    )
    ...
    if test_query:
        ...
        for t in approved_terms:
            term_name = t.get("term", "")
            synonyms = t.get("synonyms", [])
            all_terms_for_match = [term_name] + (
                synonyms if isinstance(synonyms, list) else []
            )
        ...
        if matched:
            ...
            st.dataframe(
                pd.DataFrame(matched), use_container_width=True, hide_index=True
            )
        else:
