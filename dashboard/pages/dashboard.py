    ...
    if api_failed(kbs_result) or api_failed(agg_result):
        ...
        with col4:
            st.metric(
                ...
                help="■ 산정 기준: 콘텐츠 길이(최대 60점, 로그 스케일) + 구조 보너스(테이블·코드·헤더·이미지·링크, 각 8점) + 품질 등급(GOLD +15 / SILVER +10 / BRONZE +5) ■ 등급: 90+ A(우수) · 70+ B(양호) · 50+ C(보통) · 30+ D(미흡) · 30미만 F(부족) ■ 평균 60%는 대부분 문서가 500-2000자 텍스트 위주이며 구조 요소(테이블 등)가 적어 C-B등급에 분포함을 의미합니다.",
            )
