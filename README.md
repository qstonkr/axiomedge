...
19개 admin 페이지 — 5 카테고리.

| 카테고리 | 페이지 |
|---------|--------|
| 개요 | 운영 대시보드 (24h 검색 / KB·문서·청크 통계 / 알림) |
| 콘텐츠 관리 | 데이터 소스 · Ingest 작업 · 용어집 (223k) · 담당자 · 검색 그룹 · 중복/모순 · 검증 대기 · 라이프사이클 |
| 품질·평가 | RAG 품질 (KTS 6-Signal radar) · Golden Set · Agent Trace · 오류 신고 |
| 시스템 운영 | 사용자/권한 (사용자 / KB 권한 / ABAC) · Edge 모델 (서버/프로필/베이스/빌드/큐레이션/배포 6 탭) · 작업 모니터 · 가중치 설정 |
| 그래프 | 엔티티 탐색 (검색→1-hop) · 스키마 검토 (Bootstrap/재추출) |
